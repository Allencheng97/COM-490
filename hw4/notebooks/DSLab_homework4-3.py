...
# %% [markdown]
# You construct the Key by parsing the train number and the station id from the json message and combining them. You use the message object as the Value.
#
# The K-V pair is constructed in this way because we want to compute the time a train stays at a station and we need the key to contain the identity of both the train and station to track a departure event.
...
def get_actual_time(tab):
    try:
        if tab[1].get('#text'):
            return dateutil.parser.parse(str(tab[1]['#text']), ignoretz=True)
        else:
            return None
    except IndexError:
        return None  
